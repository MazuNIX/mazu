...
The following is a set of guidelines for contributing to [mazu](https://github.com/mazunix/mazu)
...
Visit our [Issues page on GitHub](https://github.com/mazunix/mazu/issues) to search and submit.
...
* [clang-format](https://clang.llvm.org/docs/ClangFormat.html) version 22 or later.
...
Comment quality checklist (invariant-first):
* Document lock/state invariants at subsystem boundaries (scheduler, trap,
  syscall, networking state machines).
* Explain ownership and lifetime rules for pointers, buffers, and embedded
  objects.
* Prefer "why" and safety constraints over restating obvious syntax.
* Remove or update stale comments in the same patch that changes behavior.
